...
export default function Footer({ reservation = {
...
}, copyright = "© 2024 deco.cx. All rights reserved.", privacyTerms = {
    label: "Termos",
    href: "#",
}, socials = [], }: Props) {
    return (<>
      <div class="border-t border-accent w-full lg:hidden mt-10">
        <div class="flex justify-center">
          {socials.map((social) => (<a href={social.href} target="_blank" rel="noopener" class="text-neutral w-[100px] h-[100px] flex items-center justify-center">
              <Image src={social.icon} width={35} height={35} alt="social-icon" class={social.href.includes("tripadvisor")
                ? "w-[50px] h-[50px]"
                : `w-[35px] h-[35px]`}/>
            </a>))}
        </div>
        ...
        <div class="flex flex-col gap-20 max-w-[85%] mx-auto w-full py-10">
          <div class="flex flex-col gap-6 justify-between lg:flex-row">
            ...
            <div class="hidden lg:flex gap-4 items-center">
              {socials.map((social) => (<a href={social.href} target="_blank" rel="noopener" class="text-neutral w-[60px] h-[60px] flex items-center justify-center">
                  <Image src={social.icon} width={35} height={35} alt="social-icon" class={social.href.includes("tripadvisor")
                ? "w-[50px] h-[50px]"
                : `w-[35px] h-[35px]`}/>
                </a>))}
            ...
            <div class="flex flex-col gap-[25px] tracking-widest">
              ...
              <div dangerouslySetInnerHTML={{ __html: address.text }} class="flex flex-col gap-4 text-[13px] text-neutral font-extralight"/>
            </div>
          </div>
        </div>
        <div class="w-full bg-secondary">
          <div class="py-5 max-w-[85%] mx-auto w-full text-white">
            <span class="text-xs flex gap-4">
              <p>{copyright}</p>
              <IframeModal iframeLink={privacyTerms.href} buttonText={privacyTerms.label} customClass="opacity-1 hover:opacity-70 transition-all duration-300"/>
            </span>
          </div>
        </div>
      </div>
