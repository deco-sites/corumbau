...
function CookieConsent({ text = "Guardamos estatísticas de visitas para melhorar sua experiência de navegação.", policy = {
    ...
    const id = useId();
    return (<>
            <div id={id} class="transform-gpu translate-y-[200%] transition fixed bottom-0 w-screen z-50 sm:flex sm:bottom-2 sm:justify-start ml-2">
                    ...
                    <div class="p-3">
                        <div class="flex flex-col items-center sm:flex-row sm:items-end flex-wrap sm:gap-1">
                            <p class="text-[10px] text-center sm:text-start max-w-[422px]">
                                {text}
                                <span>{" "}</span>
                                <IframeModal iframeLink={policy.link} buttonText={policy.text}/>
                            </p>
                        </div>
                    </div>

                    <button class="bg-black hover:opacity-70 py-5 px-4 uppercase text-white font-bold text-[10px]" data-button-cc-accept>
                        {buttons.allowText}
                    </button>
                </div>
            </div>
