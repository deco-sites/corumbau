...
function script(id: string) {
    const handleClick = () => {
        (globalThis.document.getElementById(id) as HTMLDialogElement)
            ?.showModal();
    };
    const handleClose = () => {
        (globalThis.document.getElementById(id) as HTMLDialogElement)
    ...
    globalThis.document.getElementById(`button-${id}`)?.addEventListener("click", handleClick);
    globalThis.document.getElementById(`button-${id}-close`)?.addEventListener("click", handleClose);
}
const IframeModal = ({ buttonText, iframeLink, customClass }: Props) => {
    const id = useId();
    return (<>
            <button id={`button-${id}`} class={`text-[10px] underline text-center ${customClass}`} type="button" onClick={() => {
            console.log("test");
            (globalThis.document.getElementById(id) as HTMLDialogElement)
                ?.showModal();
        }}>
            ...
            <dialog id={id} className="modal hidden open:grid">
                <div className="modal-box p-0 rounded-none h-[90vh] md:h-[80vh] min-w-[90%] max-w-[768px]">
                    <iframe id="myIframe" src={iframeLink} class="w-full h-full">
                    </iframe>
                </div>
